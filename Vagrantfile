...
Vagrant.configure("2") do |config|
    ...
    config.vm.define "lindat-kontext-vagrant"

    # Hostname for virtual machine
    config.vm.hostname = "kontext.vagrant.dev"
    ...
    config.vm.provision :shell, :path => "projects/puppet-bootstrap-ubuntu.sh"
...
controluser_password=vagrant" > /etc/phpmyadmin.facts;'
    ...
    config.vm.provision "shell", path: "./projects/setup.munin.sh"
    config.vm.provision "shell", path: "./projects/setup.kontext.sh"
    

    # Message to display to user after 'vagrant up' completes
    config.vm.post_up_message = "Setup of 'lindat-kontext-vagrant' is now COMPLETE! Kontext should now be available at:\n\nhttp://localhost:#{CONF['port']}/kontext/\n\nYou can also SSH into the new VM via 'vagrant ssh'"
end
